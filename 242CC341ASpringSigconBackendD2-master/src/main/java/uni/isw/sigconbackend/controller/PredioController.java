...
public class PredioController {
    ...
    @GetMapping("/findruc")
    public ResponseEntity<?> findPredioByRuc(@RequestBody Optional<Predio> predio){
        logger.info(">findruc" +  predio.toString());
        PredioResponse predioResponse;
        try{
            predioResponse=predioService.findPredioByRuc(String.valueOf(String.valueOf(Long.valueOf(predio.get().getRuc()))));
        }catch(Exception e){
            logger.error("Error inesperado", e);
            return new ResponseEntity<>(null,HttpStatus.INTERNAL_SERVER_ERROR);
        }
